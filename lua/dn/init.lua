...
autocmd('LspAttach', {
    ...
    callback = function(e)
		...
		vim.keymap.set("n", "<leader>lws", function() vim.lsp.buf.workspace_symbol() end, { buffer = e.buf, desc = "Lists all symbol"})
        vim.keymap.set("n", "<leader>ld", function() vim.diagnostic.open_float() end, { buffer = e.buf, desc = "Show contents in floating window"})
        vim.keymap.set("n", "<leader>lca", function() vim.lsp.buf.code_action() end, { buffer = e.buf, desc = "Show code actions"})
        vim.keymap.set("n", "<leader>lrr", function() vim.lsp.buf.references() end, { buffer = e.buf, desc = "Show references"})
        vim.keymap.set("n", "<leader>lrn", function() vim.lsp.buf.rename() end, { buffer = e.buf, desc = "Rename reference"})
